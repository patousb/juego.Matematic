// server.js - SERVIDOR MATH CHALLENGE PRO - VERSIÓN COMPLETA Y MEJORADA
  ...
  function handlePlayerDisconnection(pin, playerId) {
    ...
    if (player) {
      ...
      if (room.hostId === playerId && room.players.size > 0) {
        const newHost = room.getPlayersArray()[0];
...
server.listen(PORT, () => {
  ...
  console.log(`   - 10 modos de juego completos con preguntas`);
  console.log(`   - Sistema de torneo con semifinales y final`);
  console.log(`   - 3 niveles de dificultad (fácil, intermedio, difícil)`);
  console.log(`   - Sistema de votación mejorado`);
  console.log(`   - Modo espectador para torneos`);
  console.log(`   - Estadísticas completas de jugadores`);
  console.log(`   - API de administración`);
  console.log(`   - Limpieza automática de salas inactivas`);
  console.log(`   - Manejo graceful de cierre`);
  console.log(`   - Logs detallados para debugging`);
  console.log(`   - Heartbeat y reconexión robusta`);
